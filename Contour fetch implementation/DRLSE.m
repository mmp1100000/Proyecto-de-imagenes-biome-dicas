function [phi,vari,area,dim] = DRLSE(Img, phi,vari)
  %DRLSE Obtains a contour via active contour algorithms making use of the drlse_edge function.
  ...
  while bucle ~= 27 %While not the end code...
      phi = phiInicial;

      [alfa,lambda,timestep,iter_outer,mu,...
       iter_inner,epsilon,g,potentialFunction] = calculations(Img,vari); %Does initial calculations needed for drlse_edge function
